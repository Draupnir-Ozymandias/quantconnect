...
class CandleStreakEntryModel:
    ...
    def get_direction(self, bar):
        ...
        if len(self.recent_directions) >= self.streak_length:
            ...
            if all_up:
                signal = "up" if self.streak_mode == "follow" else "down"

            elif all_down:
                signal = "down" if self.streak_mode == "follow" else "up"
        ...
        if len(self.recent_directions) > self.streak_length:
            self.recent_directions = self.recent_directions[-self.streak_length:]
